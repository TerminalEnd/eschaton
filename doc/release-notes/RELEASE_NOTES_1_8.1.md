...
Eschaton Core 1.8.1 is primarily a bugfix release, bringing Eschaton Core in
line with Bitcoin 0.9.3. Eschaton Core 1.8.1 also adds in support for printing
...
* Disable Transifex based translation process (not used by Eschaton Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 Eschaton Core clients
* Use HTTPS with eschchain.info
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/eschaton/eschaton/issues/134).
...
This only affects values significantly below 1 Esch, however merchants may wish to round to
the nearest Esch on all transactions for now.
