...
#if defined(NDEBUG)
# error "Eschaton cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Eschaton Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetEschcoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // Eschaton: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("eschaton-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Eschaton: BIP30 has been active since inception
    ...
    // Eschaton: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetEschcoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Eschaton: Disallow AuxPow blocks before it is activated.
    ...
    // Eschaton: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // Eschaton: We probably want to disable this
    int nLockTimeFlags = 0;
